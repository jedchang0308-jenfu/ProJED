...
assert.match(picker, /data-task-assignment-section="primary"/);
assert.match(picker, /data-task-assignment-section="collaborator"/);
assert.doesNotMatch(picker, /主責對成果|共同主責較多|主責成員（可複選）|協作成員（可複選）|清除主責|data-task-assignment-summary-rows|renderFullRoleRow/);
...
assert.match(filter, /intersects\(assignmentIds/);
