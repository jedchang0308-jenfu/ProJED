...
assert('prompt update mode remains non-forced', source.viteConfig.includes("registerType: 'prompt'") && source.viteConfig.includes('skipWaiting: false') && source.viteConfig.includes('cleanupOutdatedCaches: true'));
assert('onNeedRefresh is visible but never auto-applies', source.pwaUpdateService.includes('onNeedRefresh()') && source.pwaUpdateService.includes("status: 'update-available'") && !source.pwaUpdateService.includes('applyUpdateWhenBackgrounded') && !source.pwaUpdateService.includes("addEventListener('pagehide'"));
...
assert('normal prompt follows compact wording contract', source.appUpdatePrompt.includes('有新版本可用') && source.appUpdatePrompt.includes('一鍵更新') && source.appUpdatePrompt.includes('稍後') && source.appUpdatePrompt.includes('關閉更新提示') && !source.appUpdatePrompt.includes('一鍵更新到最新版') && !source.appUpdatePrompt.includes('RefreshCw'));
