...
check('S03', 'Fixed planning tracks and elastic content allocation are explicit', goal.includes('GOAL_PLANNING_WIDTHS') && goal.includes('owner: 112') && goal.includes('status: 64') && goal.includes('start: 96') && goal.includes('end: 96') && goal.includes('duration: 60') && goal.includes('hasElasticContentColumn') && goal.includes('showMeetingColumn ? <col'));
check('S04', 'Table minimum width follows visible columns', goal.includes('GOAL_CONTENT_COLUMN_MIN_WIDTH_PX = 220') && goal.includes('GOAL_COLLAPSED_COLUMN_WIDTH_PX = 32') && goal.includes('goalTableMinWidth') && goal.includes('GOAL_PLANNING_WIDTHS.owner'));
