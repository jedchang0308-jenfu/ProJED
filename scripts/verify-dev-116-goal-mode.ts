...
check('P23', 'goal headers use flat labels and compact task-name width while owner content has bounded Y-scroll', /<th id="goal-column-task"[^>]*>\s*任務名稱\s*<\/th>/.test(goalHeaderMarkup) && !goalHeaderMarkup.includes('<div') && goal.includes('min-w-[252px]') && goal.includes('w-[252px]') && goal.includes('minWidth: 252') && goal.includes('任務名稱、任務目的、會議紀錄、負責人、狀態、開始日期、結束日期與工期') && !goal.includes('>任務目標</th>') && goal.includes('data-goal-content-scroll') && goal.includes('GOAL_CONTENT_ROW_HEIGHT_PX') && goal.includes('overflow-y-auto'));
...
check('P27', 'goal owns task capabilities while progress stays list-only', goal.includes('useTaskPlacementController') && goal.includes('<DndContext') && !goal.includes('data-goal-task-progress') && goal.includes('<TagChip') && goal.includes('<TaskAssignmentPicker') && goal.includes('showIcon={false}') && goal.includes('data-goal-planning-control') && listNode.includes('data-task-progress-indicator') && !hierarchyLayout.includes('TaskAssignmentPicker') && !hierarchyLayout.includes('TagChip'));
