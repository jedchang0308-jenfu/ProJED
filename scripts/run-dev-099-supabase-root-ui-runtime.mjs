...
const localEnv = existsSync(localEnvPath) ? parse(localEnvPath) : process.env;
...
child.unref();
console.log(JSON.stringify({ project: root, purpose: 'CAPA-001 DEV-099 clean integrated hotfix candidate UI smoke on Supabase TEST', port, ownerPid: child.pid, cleanup: 'taskkill exact owner tree after smoke' }));
