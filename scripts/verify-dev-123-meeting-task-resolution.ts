...
const run = () => {
  ...
  assert.match(spec, /Local Candidate Implemented/);
  assert.match(spec, /RD Implementation Ready \/ Architecture Confirmed/);
  ...
  assert.match(adr, /DEV-123 Local Candidate Implemented/);
  assert.match(adr, /Architecture Confirmed/);
  assert.match(adr, /不得保留可回取的會議內容/);
  assert.match(adr, /manual retry.*transaction 內 editor/s);
  assert.match(devTask, /DEV-123.*Local Candidate Implemented/);
  assert.match(devTask, /DEV-123.*RD Implementation Ready \/ Architecture Confirmed/);
  ...
  assert.match(qa, /Local Candidate Implemented/);
  assert.match(qa, /Architecture Confirmed/);
  assert.match(qa, /RD Local Preflight PASS/);
