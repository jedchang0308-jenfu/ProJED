...
20. 子層任務 heading 只顯示目前任務的 task tag，不要重複祖先 path tags；階層由編號呈現，例如卡片層使用「2.1.1 @[卡片](task:card-id)」，子任務層使用「2.1.1.1 @[子任務](task:child-id)」。
21. rawContent 中沒有 task tag、也沒有明確決議/風險/待辦語意的人類手寫句子，不是雜訊；必須重新編排到「3. 臨時動議&其他」，不得弱化、不得省略。
22. rawContent 若一開始只有「1. 本次會議總結 / 2. 任務討論與結論 / 3. 臨時動議&其他」章節標題，這些只是可編輯 scaffold，不可當作會議內容。
...
- 沒有明確決議或下一步時不要硬寫；人類已輸入但不屬於任務清單的內容，放到「3. 臨時動議&其他」重新編排。

補充：若 tasks[].path 有多層，僅用於樹狀編號與排序；heading 只放目前任務的 task tag，後方敘述仍只描述目前任務本身的會議內容或 activity 結論。
