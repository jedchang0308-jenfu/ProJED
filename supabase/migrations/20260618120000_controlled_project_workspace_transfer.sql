...
as $$
declare
  source_id uuid := source_tenant_id;
...
begin
  ...
  select t.name into source_workspace_name from public.tenants t where t.id = source_id;
  select t.name into target_workspace_name from public.tenants t where t.id = target_id;
  ...
  if not private.current_user_can_manage_project(source_id, moving_project_id) then
    blocked_reasons := array_append(blocked_reasons, 'source_project_manager_required');
  end if;

  if not private.current_user_is_workspace_admin(target_id) then
  ...
  where tm.tenant_id = target_id
    and tm.status = 'active';

  select count(*) into preserved_member_count
  from public.project_members pm
  where pm.tenant_id = source_id
...
begin
  ...
  set tenant_id = target_id
  where dv.tenant_id = source_id
    and exists (
      select 1
      from public.documents d
      where d.id = dv.document_id
        and d.project_id = moving_project_id
        and d.tenant_id = target_id
    );

  update public.document_chunks dc
  set tenant_id = target_id
  where dc.tenant_id = source_id
    and exists (
      select 1
      from public.documents d
  ...
  set tenant_id = target_id,
      ...
      metadata = jsonb_strip_nulls(
        ...
        || jsonb_build_object('projectWorkspaceTransferAt', started_at)
      ),
      updated_at = now()
  where rsj.tenant_id = source_id
    and exists (
      select 1
      from public.documents d
  ...
  values
    (
      ...
      'projects',
      moving_project_id,
      jsonb_build_object('tenantId', source_id, 'projectId', moving_project_id, 'name', project_record.name),
      jsonb_build_object('tenantId', target_id, 'projectId', moving_project_id, 'name', project_record.name)
    ),
    (
      target_id,
      (select auth.uid()),
