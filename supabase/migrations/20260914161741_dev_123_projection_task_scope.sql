-- DEV-123: projection task links fail closed when accepted decisions become
-- stale, archived, deleted, or move outside the capture project.
-- The prior implementation used a filtering join, which could silently drop
-- an invalid link and still save the draft.  A projection must reject the
-- whole transaction so the caller can preserve the local draft and resolve
-- the stale decision explicitly.
