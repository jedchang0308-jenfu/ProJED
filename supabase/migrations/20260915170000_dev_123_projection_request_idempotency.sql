-- DEV-123: make projection request keys a durable idempotency boundary.
-- A replay with the same key and payload is a no-op; reusing a key for a
-- different payload is rejected before any record/link mutation.
...
declare
  ...
  v_projection private.meeting_draft_projections;
  v_link record;
begin
  if p_request_key is null or length(trim(p_request_key)) = 0 then
    raise exception 'PROJECTION_REQUEST_KEY_REQUIRED';
  end if;
  if p_payload_hash is null or length(trim(p_payload_hash)) = 0 then
    raise exception 'PROJECTION_PAYLOAD_HASH_REQUIRED';
  end if;
  ...
  -- Lock the projection row after the record/capture locks.  This keeps a
  -- concurrent replay in the same ordering and prevents duplicate link work.
  select * into v_projection
  from private.meeting_draft_projections
  where record_id = p_record_id
  for update;
  if found and v_projection.last_request_key = p_request_key then
    if v_projection.capture_id = p_capture_id and v_projection.payload_hash = p_payload_hash then
      return v_record;
    end if;
    raise exception 'PROJECTION_REQUEST_CONFLICT';
  end if;
