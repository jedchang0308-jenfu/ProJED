...
Spec Impact：DEV-068 最終依使用者畫面重驗，把 child dwell target 從 title slot／shrink-wrapped title `SPAN` 改為 DEV-065 完整 hover scope（主任務＋可見子樹）。Candidate 前1秒不顯示子任務藍框，只保留 standard insertion/lane/promotion；armed 後 primary-500／primary-400 藍框才與下一子階插入線同步出現並由 child intent 接管。來源卡使用與collision解耦的pointer／finger上方fixed overlay（16px gap、8px clamp、edge fallback）。Physical iPhone／Android未偵測，故不標完整mobile sign-off、release ready或已部署。
...
| `ai-doc/qa/QA-DEV-068-task-title-center-child-drop.md` | Executed / AI Browser QA-QC Passed / Physical Mobile 未充分驗證 | DEV-068 | 64/64 DEV-068 static、27/27 rendered mouse/touch、candidate零藍框、armed藍框／插入線同步、完整scope geometry、控制項矩形、同階相容、Workbench邊界、五 viewport、截圖與實機缺口。 |
| `ai-doc/qa/QA-DEV-068-coverage-matrix.md` | 68/68 AI Coverage PASS / Physical Mobile Pending | DEV-068 | 將68個風險案例逐項連到91項browser與245項static/deterministic evidence。 |
...
| `ai-doc/specs/SPEC-068-task-title-center-child-drop.md` | Implemented / AI Browser QA-QC Passed / Physical Mobile 未充分驗證 / 未 Release | DEV-068 | 定義並落地 L1／L2／L3+ complete-hover-scope geometry、1秒child-intent、candidate零藍框／standard coexist、armed藍框與下一子階定位同步；child append 回原位時顯示來源名稱並zero-write，其餘沿用desktop/mobile共用提交契約。 |
...
- DEV-027：Xmind-like 心智圖模式。
- DEV-028：四模式一致的 Trello-like 任務操作契約。
- DEV-068：進入任務完整命中範圍後，前1秒不顯示子任務藍框並保留原standard drop，滿1秒才讓完整藍框與下一子階定位同步出現；若回到原位則顯示來源名稱並zero-write，否則放開才移入exact parent。標題尾端與主任務空白都在scope內，內部控制項排除。目前AI Browser QA/QC已通過，Physical Mobile未充分驗證，未Release。
...
| `ai-doc/qa/QA-DEV-068-task-title-center-child-drop.md` | Executed / AI Browser QA-QC Passed / Physical Mobile 未充分驗證 | DEV-068 | 已驗證 complete-hover-scope geometry、1秒timing、candidate零藍框／standard coexist、armed藍框與L2/L3/L4+插入線同步、child-origin名稱預覽／zero-write、mouse/touch、preview/commit一致、衝突回歸、資料安全與五種viewport；iOS/Android實機待補。 |
...
| `ai-doc/qc/QC-DEV-068-task-title-center-child-drop.md` | AI Browser QA-QC Passed / Physical Mobile 未充分驗證 / 未 Release | DEV-068 | 記錄whole-scope、candidate零藍框、armed藍框／child insertion同步、child-origin名稱預覽／zero-write、viewport cleanup、控制項／排序／Workbench衝突根因、失敗回送RD，以及66/66 DEV-068 static、29/29核心、64/64相鄰browser、五viewport、畫面複查與實機缺口。 |
