...
| 文件 | 狀態 | 關聯 DEV | 說明 |
|---|---|---|---|
| `ai-doc/specs/SPEC-029-mobile-pan-first-touch-interactions.md` | Phase 1 Implemented / Local Automated QA Passed / Production Not Deployed | DEV-029 / DEV-028 | 定義並記錄手機 coarse pointer 的 pan-first 手勢仲裁：任務卡、子任務列、欄位與空白處短滑優先移動畫面並 suppress click-through；手機 task surface 無位移 tap 仍開任務詳情；長按才觸發任務操作選單或既有 long press flow；按鈕、輸入框、日期/依賴/負責人/標籤與 popover/modal controls 為例外。Phase 1 已完成 BoardView / Kanban / TaskWorkbench mobile overlay，Phase 2 才處理未來 mobile non-board modes。 |
| `ai-doc/qa/QA-DEV-029-mobile-pan-first-touch-interactions.md` | Local Automated Browser QA Passed / Physical Phone Supplemental Not Executed | DEV-029 | 驗證手機任務卡主體與 L2+ 子任務列不需找縫隙即可 pan，短滑不得開 `TaskDetailsModal`、rename、context menu 或 drag；L2+ 垂直/水平 pan 需產生 `scrollTop` / `scrollLeft` 位移；長按任務操作仍有效；interactive controls 不被攔截；桌機 DEV-028 click-to-details 不被破壞；記錄 25/25 browser matrix 與真機 H01-H04 未執行邊界。 |
| `ai-doc/qc/QC-DEV-029-mobile-pan-first-touch-interactions.md` | Local Automated Browser QC Passed / Physical Phone Supplemental Not Executed | DEV-029 | 記錄 RD 修正、DEV-029 static 27/27、browser matrix 25/25、L2+ `scrollTop: 0 -> 38` / `scrollLeft: 0 -> 120`、DEV-028 regression、TypeScript、build:test、diff check evidence；明確標示未執行 production deploy、Phase 2 與 physical-phone supplemental cases。 |
...
PM 治理註記：DEV-029 是 DEV-028 mobile interaction follow-up，不推翻桌機 click-to-details，也不取消右鍵 / 長按任務操作選單。Phase 1 已依使用者要求完成 RD implementation 與本機 automated/browser QC；真機回饋後已恢復手機無位移 tap 開詳情。production deploy、手機非 board modes 重新開放、再次取消或重定義手機 tap-to-details 仍需要使用者另行授權。H01-H04 physical-phone 補充案例尚未完整執行，不得宣稱真機手感已簽核。
...
| 文件 | 狀態 | 關聯 DEV | 說明 |
|---|---|---|---|
| `ai-doc/specs/SPEC-037-calendar-subscription-source-scope-clarity.md` | RD Contract Ready | DEV-037 / DEV-036 | 定義行事曆訂閱的來源範圍模型，將 `目前看板`、`工作區全部看板`、`自訂範圍` 與 `來源 / 條件` summary 分清楚；要求 filters 支援 `scope_type` / `project_ids`，並要求 Edge Function 與 DB validation 保證 Board scope 不外溢。 |
| `ai-doc/qa/QA-DEV-037-calendar-subscription-source-scope-clarity.md` | RD Contract Ready / QC Pending | DEV-037 | 驗證計畫涵蓋 UI source summary、legacy subscription、Board scope ICS feed、Workspace scope 權限、DB validation、Edge Function、mobile viewport 與 Settings/Workspace regression gates。 |
...
| `ai-doc/specs/SPEC-029-mobile-pan-first-touch-interactions.md` | Phase 1 Implemented / Local Automated QA Passed / Production Not Deployed | DEV-029 | 定義手機 BoardView / Kanban / TaskWorkbench pan-first 觸控仲裁，手機 task surface 短滑不誤開詳情，無位移 tap 仍開詳情，長按與 interactive controls 保留；Phase 2 與 production 未授權。 |
...
| `ai-doc/qa/QA-DEV-029-mobile-pan-first-touch-interactions.md` | Local Automated Browser QA Passed / Physical Phone Supplemental Not Executed | DEV-029 | 驗證手機 pan-first：任務卡、L2+ 子任務、欄位與工作台 row 短滑不誤開詳情，L2+ pan 可推動 `scrollTop` / `scrollLeft`，無位移 tap 可開詳情，長按與 interactive controls 保留，桌機 click-to-details 回歸通過。 |
...
| `ai-doc/qc/QC-DEV-029-mobile-pan-first-touch-interactions.md` | Local Automated Browser QC Passed / Physical Phone Supplemental Not Executed | DEV-029 | DEV-029 手機 pan-first 觸控仲裁事實驗證，記錄 static 27/27、browser matrix 25/25、L2+ scroll displacement、DEV-028 regression、TypeScript、build:test 與真機補充未執行邊界。 |
