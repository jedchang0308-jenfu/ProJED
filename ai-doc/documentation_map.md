...
| 文件 | 狀態 | 關聯 DEV | 說明 |
|---|---|---|---|
| `ai-doc/specs/SPEC-039-task-filter-core-and-workbench-profiles.md` | Phase 1/1A Implemented / Local Automated QC Passed / Phase 1B Implemented / Local Automated QC Passed / Phase 1C Implemented / Local Automated QC Passed / Production Release Not Deployed + Requires Explicit Authorization / Phase 2 RD Contract Ready / All-Phase Coverage Complete | DEV-039 / DEV-027D / DEV-028 / DEV-036 | 定義任務過濾器共用核心、看板任務視圖一致化、顯示設定與過濾條件分離、全域任務平台單一過濾器入口：主畫面一顆 `過濾器` 按鈕，popover 內選看板並調同看板過濾器；Phase 1B 已補回未歸位 / 已歸位看板 placement lanes、雙向拖移與未歸位任務功能等價；Phase 1C 已完成 filter result parity 實作與本機自動化 QC，要求同看板同條件下 board/workbench `matchedTaskIds` 一致且 ancestor 只作 context；明確取消 profile/storage/copy/sync，並保留 Phase 2 task source truth contract、Production Release Gate、Deferred Scope Audit 與 All-Phase Coverage Matrix。 |
| `ai-doc/qa/QA-DEV-039-task-filter-core-and-workbench-profiles.md` | Phase 1/1A QA Passed / Phase 1B QA Passed / Phase 1C QA Passed / Local Automated QC Passed / Phase 2 QA Contract Ready / All-Phase Coverage Complete | DEV-039 | 驗證計畫涵蓋共用 predicate、active filter count、五視圖一致性、Workbench 單一過濾器按鈕與 popover 內看板/過濾器、未歸位 / 已歸位看板 placement lanes、雙向拖移、任務卡功能等價、Phase 1C matchedTaskIds 結果一致、context-only ancestor、負責人 option source 對齊、禁止 profile/save/copy UI、mobile viewport gates、phase exit rules 與 deferred verification audit。 |
| `ai-doc/qc/QC-DEV-039-task-filter-core-and-workbench-profiles.md` | Phase 1/1A + Phase 1B + Phase 1C Local Automated QC Passed / DB unchanged / Production Not Deployed | DEV-039 | 記錄 DEV-039 兩欄簡化、未歸位 / 已歸位看板 placement lanes、雙向拖移、任務卡功能等價、Phase 1C result parity，以及 static/browser/regression/TypeScript/build gates；production release 未執行，後續仍需使用者明確部署授權與 deployment-release-gate。 |

PM 治理註記：DEV-039 採使用者最新一顆按鈕方案。全域任務平台是 BoardView 左側跨看板拖拉工作流，不得改成獨立整頁 route；工作台主畫面只保留 `過濾器` 按鈕，點開 popover 後才選看板並調同看板過濾器，讓使用者看板一個一個設定。Popover 內看板欄只切換正在設定哪個看板的過濾器；已歸位任務清單跨看板顯示，依各任務所屬看板套用該看板 filter state；看板 selector 不得與過濾器按鈕並列常駐在主畫面。`未歸位` 與 `已歸位看板` 是 placement lanes，不是過濾器或任務狀態；未歸位任務與已歸位任務功能等價且可雙向拖移，Phase 1B 已通過本機自動化 QC。Phase 1C 已完成實作與本機自動化 QC：同看板同條件下，看板與工作台必須以同一組 `matchedTaskIds` 作為結果真相；看板的祖先欄位 / 卡片可作 context-only container，工作台不得列為符合結果。profile、設定檔、儲存、另存、複製、全域/看板專屬 profile 已取消，不得回流到本 DEV。正式環境發布 / production release 未執行，仍需使用者明確 deployment authorization 與 deployment-release-gate。Phase 2 仍需使用者或 PM 明確授權才可做真正全部可見任務資料層。
...
| 文件 | 狀態 | 關聯 DEV | 說明 |
|---|---|---|---|
| `ai-doc/specs/SPEC-038-settings-scope-consistency-and-risk-guardrails.md` | Ready for RD | DEV-038 / DEV-036 / DEV-037 | 定義設定中心共同作用範圍 taxonomy，將目前看板、目前工作區、全域快照、外部連結、此裝置/目前帳號分清楚；特別處理備份全域匯出、目前看板匯入、目前看板回收桶與快速開啟的範圍語意。 |
| `ai-doc/qa/QA-DEV-038-settings-scope-consistency-and-risk-guardrails.md` | Ready for RD / QC Pending | DEV-038 | 驗證計畫涵蓋 Settings header、section scope summary、匯入前確認、目前看板回收桶清空確認、看板權限 target、快速開啟裝置/帳號範圍、mobile viewport 與 regression gates。 |
...
| 文件 | 狀態 | 關聯 DEV | 說明 |
|---|---|---|---|
| `ai-doc/specs/SPEC-037-calendar-subscription-source-scope-clarity.md` | Ready for RD | DEV-037 / DEV-036 | 定義行事曆訂閱的來源範圍模型，將 `目前看板`、`工作區全部看板`、`自訂範圍` 與 `來源 / 條件` summary 分清楚；要求 filters 支援 `scope_type` / `project_ids`，並要求 Edge Function 與 DB validation 保證 Board scope 不外溢。 |
| `ai-doc/qa/QA-DEV-037-calendar-subscription-source-scope-clarity.md` | Ready for RD / QC Pending | DEV-037 | 驗證計畫涵蓋 UI source summary、legacy subscription、Board scope ICS feed、Workspace scope 權限、DB validation、Edge Function、mobile viewport 與 Settings/Workspace regression gates。 |

PM 治理註記：DEV-037 是 DEV-036 Trello-like Workspace 模型在行事曆訂閱功能上的語意落地。此項不可只改文案；若要提供 `目前看板` 訂閱，資料契約與 feed filtering 必須能真正以 `project_id` 限縮任務來源。
...
| 文件 | 狀態 | 關聯 DEV | 說明 |
|---|---|---|---|
| `ai-doc/specs/SPEC-035-workspace-delete-persistence-fix.md` | Implemented / DB QC Pending | DEV-035 | 定義工作區刪除後重新整理又出現的根因與修正方案：Supabase owner-only delete RPC、前端後端成功後才移除 UI、失敗 toast、active workspace/board/localStorage cleanup。 |
| `ai-doc/qa/QA-DEV-035-workspace-delete-persistence-fix.md` | Local Automated QC Passed / DB QC Pending | DEV-035 | 驗證計畫涵蓋 static contract、local-test browser reload persistence、active workspace cleanup、Supabase owner/admin/member/viewer DB QC、failure-mode 與 regression gates。 |
| `ai-doc/qc/QC-DEV-035-workspace-delete-persistence-fix.md` | Local Automated QC Passed / DB QC Pending | DEV-035 | 記錄 DEV-035 static、browser、TypeScript、build、core regression、DEV-030 context menu regression 與 mobile 截圖證據；明確標示遠端 Supabase DB role QC 尚待 migration 套用。 |

PM 治理註記：DEV-035 是 P0 data consistency bug 交付點。此任務不重做工作區分組 UI、不新增回收桶；重點是把刪除成功定義改回「後端持久化成功」，並讓失敗可見。
...
| 文件 | 狀態 | 關聯 DEV | 說明 |
|---|---|---|---|
| `ai-doc/specs/SPEC-028-cross-mode-trello-like-task-interactions.md` | Implemented | DEV-028 | 定義清單、心智圖、看板、甘特四模式共用任務操作契約：單擊 = 選取 + 開詳情、保留 `TaskDetailsModal`、明確改名入口、新增任務桌機直接打字/手機自動命名、右鍵/長按任務選單、保留看板 Level 3+ 正面顯示與卡片資訊密度。 |
| `ai-doc/qa/QA-DEV-028-cross-mode-trello-like-task-interactions.md` | QA Plan Updated / Automated Browser Smoke Passed / Manual Click QC Pending | DEV-028 | 定義 DEV-028 Zero-Tolerance failures、四模式手動驗證矩陣、自動化 gate、MAN-028 人工親自點擊測試矩陣、ESC 關閉最上層暫時性 UI 與 QC handoff evidence；本輪已通過 DEV-028 static/browser gate 與 DEV-027B/027E 心智圖回歸，尚待人工親自點擊 QC。 |

DEV-028 已依 HCS 引導決策 1A / 2C / 3A / 4A / 5A / 6A 實作：快捷鍵採模式優先、新增任務命名桌機/手機分流、右鍵/長按統一任務選單、單擊既有任務先選取再開詳情、保留 `TaskDetailsModal`、選取視覺採最小 highlight / ring。已新增共用任務詳情入口、四模式 click-to-details、selected-state retention、explicit rename、新增任務命名與 browser verifier；使用者後續要求補人工親自點擊操作驗證，QA-DEV-028 已新增 MAN-028-001 至 MAN-028-026，下一步交 QC 執行。
...
| `ai-doc/specs/SPEC-028-cross-mode-trello-like-task-interactions.md` | Implemented / Browser Smoke Passed | DEV-028 | 定義清單、心智圖、看板、甘特四模式一致的 Trello-like 任務操作契約。 |
