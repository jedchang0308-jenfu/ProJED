...
本輪續接 `[$dev-pm] 完成所有開發任務` 時，`ai-doc/dev_task.md` 是唯一任務排序與授權入口。2026-07-07 DEV-045 行事曆訂閱篩選器建構器與即時預覽已完成 Phase 1 local Builder slice 與 DEV-045 Phase 2 local source；已補 Supabase validation migration source、Edge Function v2 feed matcher、client v2 normalizer、static QC、local-test browser QC、remote-readiness static gate 與 transaction-scoped local DB smoke。使用者已授權 Phase 3 remote Supabase / Edge / live `.ics` gate；read-only preflight 顯示 production project healthy、current `calendar-feed` version 3 active，local DB smoke 也已通過，但 `ProJED_TEST` inactive，remote DB/Edge/live `.ics` 仍需 deployment-release-gate / Supabase gate。2026-07-07 續接 discovery 進一步確認：production migration history 尚未包含 DEV-037 `20260706091804` 或 DEV-045 `20260706162052`，deployed `calendar-feed` version 3 仍是舊 workspace-only feed contract，production aggregate 訂閱 total 2 / active 2 / v2 0。2026-07-07 release-gate preflight rerun 已通過 DEV-045 static/source gates、DEV-037/039 regression、local DB smoke rollback、TypeScript、settings gate、Edge bundle、production build 與 production artifact smoke；但缺可用 Level 3 staging/branch，依 deployment-release-gate 不得自行直接套 production DB/Edge。使用者後續選擇 Supabase branch/staging path；`Supabase branch path 需 cost confirmation` 已完成 cost confirmation，branch cost `0.01344 / hourly` 已確認，但建立 `dev-045-calendar-subscription-v2-staging` 失敗，MCP 回 branch create 權限不足；未建立 branch、未套 migration、未部署 Edge、未改 production。PM 剩餘任務比對結論：需要輕量任務板重構，不需要重編 DEV；DEV-045 / DEV-037 改以行事曆訂閱 workstream 管理，DEV-025 已完成 production read-only preflight、guarded fixture-readiness harness、Execution Readiness Static Gate Added 與 guarded mutating executor self-check，mutating role-data QC 仍需安全 fixture。DEV-038 / DEV-042 / DEV-044 safe scope 已發布到 Firebase Hosting production，且 DEV-042 physical-phone supplemental 已由使用者回報通過。剩餘項目需要 Supabase DB / Edge deploy gate、已登入正式前端 UI smoke、DEV-028 人工親自點擊 QC，或其他未完成真機 supplemental。2026-07-07 已新增 PM evidence read-only audit `verify:remaining-external-gates`，用來確認這些剩餘 Gate 邊界仍一致，且不代表任一外部 Gate 完成。同日 DEV-011 / DEV-012 補上 guarded executor `verify:dev-011-012-production-ui-smoke`，預設 self-check 不登入、不建立 production 資料、不呼叫 AI；完整 fixture path 仍需 explicit fixture opt-in。
