...
### DEV-055：電腦版任務拖拉落點清晰化與跨階層定位升級（Completed / User Desktop Gate Passed / Local Complete）

| 文件 | 狀態 | 關聯 DEV | 說明 |
|---|---|---|---|
| `ai-doc/specs/SPEC-055-desktop-task-drag-target-clarity.md` | Completed / Automated QA-QC + User Desktop Acceptance Passed / Production Not Deployed | DEV-055 / DEV-053 / DEV-054 / DEV-046 / DEV-051 | 使用者 T01-T08 Attempt 1 回報同格定位線漂移與 L3+ 被推開；Rework 1 改為 fixed overlay-only indicator、overlay append hit area、sortable displacement freeze 與 rect micro-retain；2026-07-17 使用者重驗通過。 |
| `ai-doc/qa/QA-DEV-055-desktop-task-drag-target-clarity.md` | Executed / Automated + T01-T08 User Desktop Acceptance Passed | DEV-055 / DEV-053 / DEV-054 / DEV-046 | DEV-055 static 27/27、browser B01-B16 16/16、指定回歸、TypeScript 與 build 通過；B15 覆蓋 L3+ no-push 與 same-cell no-drift；T01-T08 共 38 次真實桌機重驗已由使用者回報通過。 |
| `ai-doc/qc/QC-DEV-055-desktop-task-drag-target-clarity.md` | QC Passed / User Desktop Acceptance Passed / Production Not Deployed | DEV-055 | 記錄使用者驗收失敗回送 RD、Rework 1 事實、browser/store 證據、代表截圖、回歸與 user acceptance pass；未把第一次 automated pass 誤算為完成。 |
| `ai-doc/dev_task.md` | DEV-055 Completed / User Desktop Gate Passed / Local Complete | DEV-055 / DEV-053 / DEV-054 | RD Rework 1、自動 QC 與使用者 T01-T08 重驗均通過；production deploy 尚未執行。 |

PM 治理註記：DEV-055 為新交付點且計入完成率。第一次自動化 pass 不等於完成，因使用者 T01-T08 Attempt 1 已回報失敗；RD Rework 1 自動 QA/QC 通過後，2026-07-17 使用者回報 T01-T08 重驗通過，因此本機任務可關閉。不得將手機 retain/hysteresis、action rail 或 touch lifecycle 搬到桌機；不得改變已獲使用者核准的桌機 DragOverlay、起手門檻、click/right-click、commit/undo 契約。任一 displayed/committed mismatch、ancestor fallback、same-cell drift、L3+ push、placed row 可拖或桌機手感回歸皆為 stop condition；production release 仍需另行授權。
