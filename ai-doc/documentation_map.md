...
| 文件 | 狀態 | 關聯 DEV | 說明 |
|---|---|---|---|
| `ai-doc/specs/SPEC-034-mobile-board-full-surface-pan.md` | Implemented / Browser QC Passed | DEV-034 | 定義手機看板全區拖移改善：手機仍只有看板模式，卡片、下層任務、欄位空白與看板空白都應可點住移動畫面；保留短點擊開詳情、拖曳把手、input、日期/依賴控制與 modal 互動。 |
| `ai-doc/qa/QA-DEV-034-mobile-board-full-surface-pan.md` | Browser QC Passed | DEV-034 | QA 制定的驗證計畫，包含 Zero-Tolerance failures、FMEA、scroll displacement 自動化案例、390x844 visible error sweep、input / drag handle / tap regression 與 QC 執行指令；已記錄 DEV-034 QC 通過結果。 |
| `ai-doc/qc/QC-DEV-034-mobile-board-full-surface-pan.md` | Browser QC Passed | DEV-034 | DEV-034 事實驗證報告，記錄 hook 實作、DEV-029 verifier 升級、390x844 scroll displacement、tap/input regression、DEV-031 browser regression 與 build gate。 |

DEV-034 承接使用者 2026-06-26 確認：「手機應該只有看板模式，我也是在講看板模式」。本 DEV 不解除 mobile board-only；它補強 DEV-029 mobile pan-first 的驗證缺口，RD 已升級既有 DEV-029 verifier 以量測實際 `scrollLeft` / `scrollTop` 變化。
...
| `ai-doc/specs/SPEC-034-mobile-board-full-surface-pan.md` | Implemented / Browser QC Passed | DEV-034 | 定義手機看板全區拖移改善，維持 mobile board-only 並補強看板內容區 pan gesture。 |
...
| `ai-doc/qa/QA-DEV-034-mobile-board-full-surface-pan.md` | Browser QC Passed | DEV-034 | 驗證手機看板卡片、下層任務、欄位空白與看板空白 pan 造成實際 scroll displacement，且不破壞短點擊、input、拖曳把手與 visible error gate。 |
...
| `ai-doc/qc/QC-DEV-034-mobile-board-full-surface-pan.md` | Browser QC Passed | DEV-034 | DEV-034 手機看板全區拖移事實驗證，確認卡片、下層任務、欄位空白與看板空白 pan 有實際 scroll displacement，且短點擊與 input regression 通過。 |
