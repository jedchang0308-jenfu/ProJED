...
| `RecordsView`／DEV-016 條列清單 | Implemented / browser B00-B19 21/21 PASS | DEV-016／DEV-093 | 保留單一「紀錄庫」頁面與可掃描條列；頁內以同層、分區的 `收藏任務`、`會議紀錄`、`個人工作紀錄` 隔離資料，不新增頂層頁、混合「全部」清單、重複頁首或跨分區混排；tabpanel 以對應 heading 建立 `aria-labelledby` 關聯，典藏詳情另以共用任務內容元件呈現唯讀快照。 |
