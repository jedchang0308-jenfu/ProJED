...
| `ai-doc/specs/SPEC-068-task-title-center-child-drop.md` | Implemented / AI Browser QA-QC Passed / Physical Mobile 未充分驗證 / 未 Release | DEV-068 | 定義並落地 L1／L2／L3+ complete-hover-scope geometry、1秒child-intent、candidate零藍框／standard coexist、armed藍框與下一子階插入線同步，以及desktop/mobile共用提交契約。 |
...
- DEV-027：Xmind-like 心智圖模式。
- DEV-028：四模式一致的 Trello-like 任務操作契約。
- DEV-068：進入任務完整命中範圍後，前1秒不顯示子任務藍框並保留原standard drop，滿1秒才讓完整藍框與下一子階插入線同步出現，放開才移入exact parent；標題尾端與主任務空白都在scope內，內部控制項排除。目前AI Browser QA/QC已通過，Physical Mobile未充分驗證，未Release。
...
| `ai-doc/qa/QA-DEV-068-task-title-center-child-drop.md` | Executed / AI Browser QA-QC Passed / Physical Mobile 未充分驗證 | DEV-068 | 已驗證 complete-hover-scope geometry、1秒timing、candidate零藍框／standard coexist、armed藍框與L2/L3/L4+插入線同步、mouse/touch、preview/commit一致、衝突回歸、資料安全與五種viewport；iOS/Android實機待補。 |
...
| `ai-doc/qc/QC-DEV-068-task-title-center-child-drop.md` | AI Browser QA-QC Passed / Physical Mobile 未充分驗證 / 未 Release | DEV-068 | 記錄whole-scope、candidate零藍框、armed藍框／child insertion同步、viewport cleanup、控制項／排序／Workbench衝突根因、失敗回送RD，以及64/64 DEV-068 static、27/27核心、64/64相鄰browser、五viewport、畫面複查與實機缺口。 |
