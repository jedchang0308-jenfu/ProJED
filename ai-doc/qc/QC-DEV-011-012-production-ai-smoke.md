...
日期：2026-06-09
狀態：Backend Pass / UI Readiness Gate Added / Production UI Smoke Executor Added / UI Pending
...
完整前端 UI smoke 尚未完成。2026-07-07 已新增 read-only readiness gate `verify:dev-011-012-production-ui-smoke-readiness`，確認 repo 已有 authenticated session injection + cleanup pattern（見 DEV-040 production authenticated UI smoke）與 local AI整理 browser ROT。該 gate `mutates_database=false`，不登入、不建立 production fixture、不呼叫 AI。2026-07-07 另補 guarded executor `verify:dev-011-012-production-ui-smoke`；預設 self-check 仍是 `mutates_database=false`，完整 production fixture path 必須同時使用 `--run-production-fixture` 與 `DEV011012_ALLOW_PRODUCTION_FIXTURE=1`，才會建立/清理 production 臨時 user / tenant / board / record fixture 並操作正式前端 meeting mode、AI整理、發布、紀錄庫與任務知識 UI。完整 UI smoke 仍需使用已登入 Google 的互動式 browser QC，或另行顯式允許 production fixture path 後執行。
...
- Production UI smoke guarded executor self-check：Pass；確認 executor 預設不登入、不建立 production 資料、不呼叫 AI，且完整 fixture path 必須同時使用 `--run-production-fixture` 與 `DEV011012_ALLOW_PRODUCTION_FIXTURE=1`，並包含 cleanup、紀錄庫、任務知識與 `knowledge_records` / `record_task_links` 查證。
