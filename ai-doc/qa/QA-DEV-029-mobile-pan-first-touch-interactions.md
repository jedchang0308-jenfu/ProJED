...
狀態: Local Automated Browser QA Passed / Physical Phone Supplemental Not Executed
建立日期: 2026-07-04
...
| ID | 情境 | 前置條件 | 操作 | 預期結果 | 證據 |
|---|---|---|---|---|---|
| QA-029-C01 | 任務卡靜止長按 | 任務卡可見 | touchstart 後靜止 500-650ms，再 touchend | 開任務操作選單或既有 long press action | menu screenshot |
| QA-029-C02 | 子任務列靜止長按 | checklist row 可見 | 靜止長按 | 開任務操作選單或既有 long press action | menu screenshot |
...
| QA-029-C05 | context menu viewport safety | C01 後 | 觀察 menu bounding box | menu 不超出 390x844，不遮住到不可操作 | screenshot + bounding box |
...
| QA-029-H04 | 軟鍵盤影響 | 真機 | 新增 / rename 任務後鍵盤出現 | 鍵盤不造成主要 controls 被遮到不可操作 | 截圖 / 錄影 |
...
| QA-029-C01 | Task card | long press 450-550ms, move < tolerance | 開任務操作選單或既有 long press action |
...
Coverage note:
- `verify:dev-029-mobile-pan-first-interactions-browser` 必須覆蓋 A01-A04、B01-B08、C01-C05、D01、D03、E01-E05、F01-F03 的可自動化部分。
...
| 長按失效 | pan guard 永遠攔截 task surface | DEV-028 任務操作入口消失 | C01/C02 | P0 | 靜止 500-650ms 長按，確認 menu |
...
- 操作情境矩陣逐項 pass/fail，至少列出 A/B/C/D/E/F/G/H 分類。
