...
- `wrapProjectChangeImportContent` 與既有 AI整理 merge guard 回歸。
...
| 插入後破壞 DEV-021 preserve guard | 改 UI 時繞過 `wrapProjectChangeImportContent` | AI整理或保存時丟失匯入內容 | 跑 DEV-021 verifier，並真實操作匯入後 AI整理 | P1 | TC-006、TC-009、ROT-003 |
...
| TC-006 | Manual / Automated | 插入後狀態可辨識 | 有 project changes 時執行整理與插入 | 內容經 `wrapProjectChangeImportContent` 插入；`匯入` step 顯示已插入與事件數；面板收合。 |
...
- `wrapProjectChangeImportContent` 仍為插入 protected content 的唯一入口。
