...
驗證 ProJED 心智圖新增任務、選取、命名、縮放、線條與拖曳 preview 是否符合 Xmind-like 操作。此版 QA 以使用者最新要求為準：新增任務後不得自動開啟 rename input；新任務只被選取，使用者可以方向鍵移動選取，並可直接打字改名。
...
| ZT-027B-006 | 直接打字沒有進入 rename，或第一個字元被吃掉 | rename input value + final title |
| ZT-027B-007 | `Enter` 在 rename input 中又建立新任務 | node count before/after |
...
| UI-027B-007 | 1440x900 | 選任務後直接輸入文字 | 進入 rename mode，第一個字元保留，Enter 只 commit |
| UI-027B-008 | 1440x900 | rename input 內按 `Enter` | 只完成改名，不新增 sibling |
