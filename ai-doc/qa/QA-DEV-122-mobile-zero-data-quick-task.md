...
- 狀態：`QA Candidate / Architecture Frozen / Partial Evidence Recorded / Full QA-QC and WP-122-0 Target-device Gate Pending`。
- 修訂：Tech Lead R10 + RD local candidate execution + external gate handoff；static/pure、quick browser（含 `install=1` 快速安裝引導 smoke）、root intent browser、build、service-worker、isolated PostgreSQL core matrix與 DEV-097 browser 已有可追溯 PASS，補記並收斂 DEV-097 verifier 的實際相容回歸修正，完整case沒有冒充通過。
...
- Risk lane：Medium；新增 mobile entry、PWA identity、voice permission、local transaction、auth claim及 UI/API/DB delivery path。
- 驗證角色：RD 先在同一 candidate執行 self-test；QA 依本文件重跑；QC凍結 source/artifact後只觀察事實，不修改產品。

## 1. 驗證目標與正常 delivery path

證明使用者從第二個手機桌面圖示進入 `/quick-task/` 後，原生 HTML 的任務名稱欄位立即可編輯，旁邊直接可見「語音」；建立先以 IndexedDB transaction/readback形成 durable local fact，再以同一 capture id經 Supabase RPC成為登入帳號全域任務工作台的 canonical未歸位任務。
...
D01～D04 才是 WP-122-0 的必要案例；D02 的安裝手勢只適用 iOS，D03 只適用 Android，對側記 N/A，兩平台都須通過第二圖示／正確 launch 結果。D05～D07 不阻塞尚未實作其功能的 WP-122-0，但必須於後續指定 WP／最終真機驗收通過。缺裝置／HTTPS candidate 為 Not verified，不記平台 Fail。安裝同一 icon或root deep link不能代替第二 App；真正的平台限制回ADR，implementation bug先回RD修正。
...
| S03 | root manifest `id='/'`；quick manifest `id/start_url/scope='/quick-task/'`、name正確、180/192/512 icons存在；quick raw HTML含manifest、apple touch icon、standalone capable、app title與theme color標記。 |
...
- Case mapping：quick browser artifact 使用 `caseSet=local-smoke-v2`、`SM01`～`SM15`；reused Vite test server shell version 為 `build:1789392291929-8mqkr19f`；這 15 個 smoke case 是本地可重現切片，不直接宣稱 QA 表中的 B01～B21 已完成。SM05 證明 unsupported fallback 後再次點擊語音可重試（`retryValue=再試語音重試`）；SM14 證明 Chromium 的原生 prompt 呼叫與安裝模式畫面，SM15 證明 iOS UA 分支的加入主畫面指引，三者均不能替代 iOS／Android install promotion。
...
- Not verified：iOS／Android安裝與真機語音、OAuth callback與正式Supabase；本機 readiness 檢查顯示 ADB 與 Xcode simulator 不可用，測試 HTTPS host `/quick-task/` 回 404，尚無 approved HTTPS candidate；P07 混入真實既有外部 writer／placement 的完整 transport 競態、P10 placement、P11 advisor review、P12/P13 完整 API／cascade matrix、P14 完整 fault injection；B01～B21 的完整 QA case、W01～W06 與獨立 QA/QC。quick／existing placement lock key alignment 已由 S11-lock-scope PASS 證明，且 40-client mixed-writer-compatible fixture 已取得 40 unique orders；fixture 使用 test-only existing append function，仍不等同 P07 真實 mixed-writer placement transport PASS。`local-smoke-v2` 僅覆蓋輸入／保存／fallback／IME／voice insertion／零業務 request／延遲 module／outbox lease／claim slice，不能替代完整 auth／transport／recovery／裝置 case。P07 固定 fixture 的 pgbench concurrent transport 已 PASS；P11 owner/order index 與 max-order EXPLAIN、P13 profile cascade 已在 22-check core matrix PASS，但不代表完整 advisor、API 或角色矩陣。isolated PostgreSQL與pgbench均使用task-owned loopback runtime，沒有遠端DB mutation。唯讀檢查既有 Docker Supabase runtime 僅確認未載入 DEV-122 migration／placement RPC，未對該既有 runtime 套用變更；因此不能把它當作 DEV-122 P10 或正式 OAuth 證據。DEV-097 browser 已在 2026-09-14 以修正後 verifier PASS；診斷與修正歷程見 `output/qa/dev-122/dev-097-compatibility-diagnostic.json` 及 `output/playwright/dev-097/ui-result.json`：原始落差是驗證器未先展開預設收合側欄，且將會議模式 local recovery snapshot 的 IDB 保存誤當 canonical record owner safe；修正後以明確 user-confirmed canonical boundary 完成 recovery。這不改 DEV-122 quick path 的架構，也不放寬 fail-closed oracle。
...
- 真機最小組合：一台 iPhone（Safari，記錄 iOS 版本）與一台 Android（Chrome，記錄 OS／Chrome 版本）；兩台都使用同一個 frozen candidate 的 HTTPS URL。沒有真機時，D01～D07、permission、storage 與 install promotion 必須維持 Not verified。
...
- 裝置操作：D01～D04 先確認兩個桌面 icon、quick launch pathname、storage sentinel；D05～D07 再依 offline、microphone permission、OAuth callback／cancel／expired nonce、關閉重開與同一 capture replay 順序執行。
...
- 完成判定：只有 D01～D07、B01～B21、W01～W06、P01～P14、效能目標與獨立 QA/QC 全部在同一 frozen candidate 通過，才可把 DEV-122 從 Candidate 改為 PASS；local-smoke-v2、單張 screenshot、UA mock 或單一 DB matrix 只能作為補充證據。
## 11. Pass／Fail／Stop

- `WP-122-0 PASS`：D01～D04依平台適用性在同一candidate通過，資源清理可追溯；只解除後續實作gate。
- `DEV-122 PASS`：S01～S14、B01～B21、W01～W06、P01～P14、效能目標、既有DEV-096／097／115受影響regressions及D01～D07依平台適用性在同一收斂candidate全通過。
...
- `Stop and return to planning`：命中SPEC第11/18節的架構限制、需要第二worker／origin、超出已定API/schema/RLS/state/ownership、放寬existing oracle，或平台確實不能形成第二App。一般bug回RD；本SPEC預定RPC/receipt migration本身不算drift。
