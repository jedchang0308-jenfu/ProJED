...
- 使用者已確認 Workbench `placed row` 不能拖。
- 本 QA 中 Workbench placed row 可拖回 unplaced 或 reorder 的案例已被 `SPEC-053` 覆寫，不再作為目前驗收條件。
- DEV-046 的 whole-task surface 驗證仍適用於看板卡片、checklist row、WBS list row、task-backed column/header 與 Workbench `unplaced row`。
- Workbench `placed row` 的目前驗收方向改為 read-only placement list entry：可點擊 / 右鍵 / 非 placement 操作，但不得暴露 draggable root 或造成 placed -> unplaced persistence。
...
確認電腦與手機上，所有明確納入的 task-backed surface 都從「拖曳把手限定」改為「整個任務本體可拖曳」。這裡的「所有任務」包含所有階層: 父任務、子任務、孫任務、Level 3+ checklist row、Level 4/5 deep row 與全域任務平台中的未歸位 task row；Workbench placed row 依 DEV-053 不能拖，不再納入 whole-surface drag 驗收。驗證同時不得破壞:
...
- DEV-039 workbench unplaced row drag 與 placed row read-only placement contract。
...
- DEV-039 static/browser: 全域任務平台未歸位 row-root drag、placed row read-only contract、右鍵 shared menu 與 placement lane 回歸。
...
- `TaskWorkbenchPanel` 維持未歸位 row root drag surface，且 Workbench placed row 不暴露 draggable root。
...
| AUTO-046-D06 | Workbench placed row | 從 row root 三點嘗試拖回 unplaced 或 reorder | no placement drag；no duplicate；no local unplaced persistence；click/right-click 不回歸 |
...
| AUTO-046-M08 | Workbench unplaced row | long press | compact action rail + drag preview |
| AUTO-046-M13 | Workbench placed row | long press / drag attempt | no placement drag; no placed -> unplaced persistence; non-placement actions do not imply draggable row |
...
| MAN-046-008 | 1440x900 | Workbench placed/all-task | 從 hierarchy padding、title、日期附近嘗試拖曳 | 已歸位 row 不進入 placement drag；不得拖回未歸位或產生 duplicate | video + before-after data |
