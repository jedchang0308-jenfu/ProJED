...
- 狀態：`RD Local Preflight PASS / Architecture Confirmed / Local Candidate Implemented / Full QA NOT EXECUTED / Provider Qualification Gate Pending`
...
Hosted gate 必須另外證明同一個 `private` API schema exposure 已部署且只有 service-role 可讀／執行，並重跑相同 control API／Storage readback；不能只因 local `supabase/config.toml` 通過就視為 hosted ready。

Hosted read-only readiness probe（`npm run verify:dev-123-hosted-readiness`）已針對 linked ProJED project 執行：remote schema lint 為 PASS，但 9 份 DEV-122／DEV-123 migration 尚未出現在 remote history，`meeting_capture_control`、`process_meeting_analysis`、`purge_meeting_audio` 亦尚未部署；public DEV-123 table readback 為 404、private schema readback 為 406。artifact：`output/qa/dev-123/hosted-readiness-result.json`。另以 `supabase db push --dry-run --project-ref knodlkxqpcqyrtgwpdst` 確認待套用範圍恰為該 9 份 migration，未包含其他 seed／role mutation。此結果明確維持 `PENDING`，未執行任何 remote mutation。
