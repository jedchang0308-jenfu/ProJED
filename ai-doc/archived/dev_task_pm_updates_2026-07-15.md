# ProJED dev_task PM Update Archive - 2026-07-15

來源：`ai-doc/dev_task.md` 第 195 行之後的歷史 `PM Update` 段落。

用途：保留 DEV 詳細 PM 更新、歷史 release gate、驗證證據與決策脈絡；active `dev_task.md` 只保留總任務清單與目前交付邊界，避免冷啟動時整份載入 3000+ 行。

讀取規則：只有追查特定 DEV 歷史、release evidence、父子依賴或 cross-task consistency 時，才搜尋並讀取本檔相關段落；不得預設整份載入。
...
### DEV-045: Production release與Level 4執行

狀態：Production Released / 38-of-38 Migration Aligned / Edge v4 / Firebase Live / Level 4 Passed
...
- Production 11個history-only repair已執行且repair前後schema hash一致；5個真實pending migration已套用，最終38/38一致且dry-run up to date。
- Production `calendar-feed` version 4逐檔等於repo；Firebase live release `1783943465159000`載入`index-DGur8aYq.js`。
...
- Level 4 production smoke通過：authenticated v3 UI 4事件與live ICS 4 VEVENT identity一致；disabled / enabled / regenerated token契約通過；random token 404；既有v1 Google Calendar事件可見；fixture residual為0。
...
- DEV-025 當時仍是 `Implemented / DB QC Pending`，先前漏列在交付點總覽與近期 Gate；本輪只修正任務板索引，不新增功能。2026-07-07 已補 production read-only preflight、guarded fixture-readiness harness、execution-readiness static gate 與 guarded mutating executor self-check，mutating role-data QC 仍待安全 fixture。
- 2026-07-07 續接已補 DEV-025 guarded fixture-readiness harness 與 guarded mutating executor；這些 gate 用來確認 production/staging fixture 有明確 `DEV-025` / `QC-DEV-025` 標記、最小資料形狀與 mutation opt-in，未完成實際 mutating role-data QC。
...
- Authorized / Complete: Phase 1 local Builder source，包括 v2 local type contract、Builder UI、本地 preview、board override / exclude、DEV-045 static/browser verifier、QC report。
...
- Authorized but Not Executed: Supabase remote migration apply、Supabase Edge Function deploy、production deploy、live `.ics` smoke、正式資料修復、Supabase live / production browser screenshot QC；local-test browser QC、read-only preflight、remote-readiness static gate 與 transaction-scoped local DB smoke 已完成。
- 2026-07-07 續接 read-only discovery：production `ProJED` / `knodlkxqpcqyrtgwpdst` 為 `ACTIVE_HEALTHY`、Postgres `17.6.1.121`；`ProJED_TEST` / `fhisnnufoeulxqrchldf` 為 `INACTIVE`。production migration history 尚未包含 DEV-037 `20260706091804_calendar_subscription_source_scope` 或 DEV-045 `20260706162052_calendar_subscription_v2_filters`；deployed `calendar-feed` version 3 仍是舊 workspace-only feed contract；production aggregate 訂閱 total 2、active 2、v2 0。Supabase branch listing MCP call 失敗，建立 branch 需另走 cost confirmation；未建立 branch、未套 migration、未部署 Edge。
- 2026-07-07 remote readiness static gate：新增並通過 `verify:dev-045-calendar-subscription-remote-readiness`，21 pass / 0 fail；只驗證本機 migration / Edge / client / governance artifacts、local DB smoke gate registration 與 stop condition，不連線 Supabase、不套 migration、不部署 Edge。
- 2026-07-07 local DB smoke：新增並通過 `verify:dev-045-calendar-subscription-local-db-smoke` self-check；actual mode `node scripts/verify-dev-045-calendar-subscription-local-db-smoke.mjs --run-local-db` 通過 12 pass / 0 fail，transaction + rollback 驗證 DEV-037 / DEV-045 DB validators、v2 snapshot project scope、missing `project_ids` negative case、board override out-of-scope negative case 與 authenticated/anon grants。此 gate 不套 remote migration、不部署 Edge、不代表 live `.ics` parity 完成。
- 2026-07-07 release-gate preflight rerun：`verify:dev-045-calendar-subscription-remote-readiness` 21/21、`verify:dev-045-calendar-subscription-v2-feed` 18/18、`verify:dev-045-calendar-subscription-builder-preview` 16/16、DEV-037 20/20、DEV-039 61/61、local DB smoke 12/12 rollback、TypeScript noEmit、settings gate、Edge bundle check 與 production build 全部通過；production artifact smoke 以 `dist/index.html` 載入 `assets/index-Gis551LA.js` / `assets/index-wbfiQlLo.css` 通過並清理臨時 preview server。Supabase read-only discovery 再次確認 production 尚未套 DEV-037/045 migrations、`calendar-feed` version 3 仍是 rollback target、active subscriptions 2 / v2 0；`ProJED_TEST` inactive 且 branch listing 失敗。依 deployment-release-gate，缺 Level 3 staging/branch 時不得自行直接套 production DB/Edge；Supabase branch path 需 cost confirmation，下一步需使用者選擇 Supabase branch cost-confirmation path 或明確接受無 Level 3 的 production risk path。
- 2026-07-07 Supabase branch cost path：使用者選擇 branch/staging path 並確認 `JED` organization。另一個 Codex session 取得 branch cost `amount=0.01344`、`recurrence=hourly`、工具未附幣別；本 session 依使用者「請繼續開發」呼叫 `_confirm_cost` 成功。隨後嘗試建立 `dev-045-calendar-subscription-v2-staging` branch 失敗，Supabase MCP 回 `You do not have permission to perform this action`。未建立 branch、未套 migration、未部署 Edge、未改 production；下一步需使用具 branch create 權限的 Supabase 帳號/OAuth，或由具權限者建立 branch 後提供 branch project ref。
- Gate compatibility note：DEV-045 仍維持 `Local DB Smoke Passed / Remote DB-Edge-Live Gate Pending`；`Supabase branch path 需 cost confirmation` 已完成 cost confirmation，但因 branch create 權限不足仍停在 deployment-release-gate / Supabase gate。production 仍缺 DEV-037/045 migrations，`calendar-feed` version 3 未含 v2 matcher。
...
- DEV-045 browser Builder preview verifier 已新增並通過，覆蓋 desktop / mobile / empty preview / board exclude / board custom override / horizontal overflow。
- DEV-045 v2 feed static verifier 已新增並通過；live `.ics` parity smoke 仍待 remote DB/Edge gate。
- DEV-045 remote readiness static verifier 已新增並通過，覆蓋 migration ordering、v2 validation grants、client normalizer、local Edge matcher、package script 防呆與文件 stop condition。
...
- 需要 remote DB migration、RLS/RPC、Edge deploy、production deploy 或正式資料修復但缺 deployment-release-gate、live smoke 或 rollback evidence。
...
- Production release / live smoke: Same Spec Phase / Phase 3；已授權，local Supabase DB smoke 已完成；remote migration apply、Edge deploy、live `.ics` preview/feed parity 與 rollback evidence 仍需 deployment-release-gate / Supabase gate。
...
| Phase 1 | Authorized | Phase 1 Local RD Implemented / Static + Browser QC Passed | Builder UI、global filter、board overrides、live local preview、v1 compatibility | Remote migration、Edge deploy、production | 使用者授權 RD | Builder preview source 可操作且不破壞 v1 source-scope UI | DEV-045 static/browser、DEV-037/039/settings regression、TypeScript、settings gate、build、QC-DEV-045 |
...
| Phase 3 | Authorized / Remote Gate Pending | Remote DB-Edge-Production Gate Pending / Read-only Discovery + Remote Readiness Static + Local DB Smoke Passed + Branch Creation Permission Blocked 2026-07-07 | Remote migration apply、Edge Function deploy、production live smoke | Unscoped feature additions | Phase 2 source passed + Supabase/deployment gate + local DB smoke passed；branch cost confirmed but branch create needs a Supabase account with branch creation permission | Production link outputs only allowed previewed tasks | remote readiness preflight、local DB smoke、deployment-release-gate、live feed smoke、Supabase MCP read-only evidence |
...
| 階段 | Owner | 狀態 | 輸出 |
|---|---|---|---|
| PM/RD 規格 | PM/RD | Implemented / DB Read-only Preflight Passed / Fixture + Execution Readiness Gates Added / Guarded Mutating Executor Added / Mutating QC Pending | SPEC-025 |
| QA 驗證計畫 | QA | Static QA Done / DB Read-only Preflight Passed / Fixture + Execution Readiness Gates Added / Guarded Mutating Executor Added / Mutating QC Pending | QA-DEV-025 |
...
| QC 事實驗證 | QC | DB Read-only Preflight Passed / Fixture + Execution Readiness Gates Added / Guarded Mutating Executor Added / Mutating QC Pending | 正式 DB 已具備 RPC / grants / constraints；已新增 read-only fixture readiness harness、execution-readiness static gate 與 guarded mutating executor self-check；待安全 fixture 上驗證 RLS、audit log、data consistency、RAG visibility evidence |
...
- Mutating QC readiness 2026-07-07: `verify:dev-025-mutating-qc-readiness` added to prevent direct package-level move RPC execution and preserve safe fixture / rollback / cleanup / RAG visibility stop conditions before any mutating DB QC.
- Guarded mutating executor 2026-07-07: `verify:dev-025-mutating-qc-execution` added. Default mode is self-check only; actual move requires `--run-mutating-fixture`, `DEV025_ALLOW_MUTATING_QC=1`, `DEV025_QC_FIXTURE_DISPOSABLE=1`, safe fixture IDs and role tokens. No mutation was executed in this file update.
