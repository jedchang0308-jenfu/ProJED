...
| Phase 3 | Authorized / Remote Gate Pending | Remote DB-Edge-Production Gate Pending / Read-only Discovery + Remote Readiness Static + Local DB Smoke Passed 2026-07-07 | Remote migration apply、Edge Function deploy、production live smoke | Unscoped feature additions | Phase 2 source passed + Supabase/deployment gate + local DB smoke passed；branch path requires Supabase cost confirmation | Production link outputs only allowed previewed tasks | remote readiness preflight、local DB smoke、deployment-release-gate、live feed smoke、Supabase MCP read-only evidence |
...
|---|---|---|---|---|
| 1 | DEV-045 Phase 3 remote Supabase / Edge / live `.ics` gate | Authorized / Local DB Smoke Passed / Release-Gate Preflight Passed / Remote DB-Edge-Live Gate Pending | Supabase / deployment-release-gate / QC / 使用者決策 | Local transaction-scoped DB smoke、static/source gates、TypeScript、settings gate、Edge bundle、production build 與 production artifact smoke 已通過；下一步只能在使用者選擇後繼續：A. Supabase branch / staging path，先完成 cost confirmation，再做 Level 3 gate；B. 使用者明確接受無 Level 3 production DB/Edge 風險後，直接 apply migration / deploy Edge 並立刻 live smoke；目前 `ProJED_TEST` inactive，production 仍缺 DEV-037/045 migrations 且 `calendar-feed` version 3 未含 v2 matcher。 |
...
| DEV-045 | 交付點 | Phase 3 Remote Gate Authorized / Local DB Smoke Passed / Release-Gate Preflight Passed / Remote DB-Edge-Live Gate Pending | 是 | 行事曆訂閱篩選器建構器與即時預覽 | `SPEC-045`、`QA-DEV-045`、`QC-DEV-045`、`verify:dev-045-calendar-subscription-builder-preview`、`verify:dev-045-calendar-subscription-builder-preview-browser`、`verify:dev-045-calendar-subscription-v2-feed`、`verify:dev-045-calendar-subscription-remote-readiness`、`verify:dev-045-calendar-subscription-local-db-smoke`、Supabase read-only preflight、local DB smoke、TypeScript、settings gate、Edge bundle、production build、production artifact smoke | 下一步需使用者選擇 Supabase branch cost-confirmation path，或明確接受無 Level 3 production risk path 後才能 remote migration / Edge deploy / live `.ics` smoke |
