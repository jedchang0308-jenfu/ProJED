...
- 保留 `## 本次會議總結`、`## 任務討論與結論`、`## 待校稿項目` 三個大章節。
- 保留 `@[title](task:id)` token 與每任務 `###` heading，確保 DEV-008 任務知識片段抽取仍可用。
...
- 正式環境 Edge Function 預設模型改為 `gemini-3.5-flash`，並保留 env override。
...
- 更新 `supabase/functions/synthesize_meeting_record/index.ts`：
  ...
  - 預設模型改為 `gemini-3.5-flash`。
  - 模型 unavailable / not found 時回傳可理解錯誤，前端沿用 DEV-011 保留原草稿行為。
...
- AI/fallback 草稿保留三個大章節與 task tag。
...
- Edge Function source 預設模型為 `gemini-3.5-flash`，且 env override 仍有效。
- AI 失敗或模型不可用時不覆蓋原草稿。
