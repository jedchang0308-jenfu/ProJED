...
- 保留 `@[title](task:id)` token；任務段落以階層編號呈現，例如 `2.1 @[列表](task:id)`、`2.1.1 @[卡片](task:id)`、`2.1.1.1 @[子任務](task:id)`，確保 DEV-008 任務知識片段抽取仍可用。
...
- 任務 heading 必須使用階層編號與 task tag，例如 `2.1 @[列表](task:id)`、`2.1.1 @[卡片](task:id)`。
