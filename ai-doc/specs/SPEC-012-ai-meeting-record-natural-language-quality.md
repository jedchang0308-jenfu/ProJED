...
- 保留 `@[title](task:id)` token；任務段落以階層編號呈現，且每個子層 heading 只顯示目前任務的 task tag，例如 `2.1 @[列表](task:id)`、`2.1.1 @[卡片](task:id)`、`2.1.1.1 @[子任務](task:id)`，階層由編號承擔，不重複祖先 path tags，且 DEV-008 任務知識片段抽取仍可用。
...
- 任務 heading 必須使用階層編號與目前任務的 task tag，例如 `2.1 @[列表](task:id)`、`2.1.1 @[卡片](task:id)`。
