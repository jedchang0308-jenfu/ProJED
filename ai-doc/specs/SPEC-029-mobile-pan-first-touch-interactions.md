...
狀態: Phase 1 + Phase 1B Implemented / Local Automated QA Passed / Production Not Deployed / Canvas CTA Pan Pass-Through Covered
對應 DEV: DEV-029
節點類型: 交付點
...
- 2026-07-17 使用者回報：手機模式下按住畫面移動的可選擇窗口未涵蓋全任務畫面，按到「新增任務」按鈕時無法移動畫面；要求盤點同型缺口並一起修正。
...
- 表單、輸入框、popover、modal、日期/依賴/負責人、filter 與 action rail 等精準控制為例外，不得被 pan-first 攔截。
- 大型畫布 CTA 例外中的例外：欄位內「新增任務」、看板尾端新增 CTA、TaskWorkbench 未歸位新增 CTA 仍保留 tap 行為，但短滑必須 pass through 到 mobile pan broker，不能成為畫面平移死角。
...
- 拖到刪除只開確認，不直接刪除。
- 點擊輸入框、日期、依賴、負責人、filter popover、modal 與 action rail 等精準控制仍能正常操作。
- 欄位內「新增任務」、看板尾端新增 CTA、TaskWorkbench 未歸位新增 CTA 可 tap 執行原動作；若從同一按鈕按住短滑，應移動 board/column surface，不新增任務、不開 modal、不進 action rail。
...
1. `touchstart` / `pointerdown`
   ...
   - interactive target 包含 `input`、`textarea`、`select`、一般 `button`、`a`、`[role=button]`、日期/依賴/負責人控制、popover/modal 內控制、drag explicit handle。
   - 若 target 或其 ancestor 標示 `data-mobile-pan-pass-through="true"`，代表該大型畫布 CTA 可 tap，但 short pan 不應被視為一般 interactive block，需交給 pan broker 判斷。
...
4. `touchend` / compatibility click
   ...
   - interactive target 例外，仍執行原本 click/tap 行為；`data-mobile-pan-pass-through="true"` target 則只在無位移 tap 時執行原 click，發生 short pan 時必須 suppress click-through。
...
- 大型畫布 CTA 若同時需要 tap 與 pan，必須標示 `data-mobile-pan-pass-through="true"`，不得只靠 `button` interactive guard；目前覆蓋 `data-kanban-add-task-button`、`data-kanban-add-column-button`、`data-task-workbench-unclassified-add`。
- CSS:
  - task/card mobile main surface 應允許 `touch-action: pan-x pan-y` 或等效。
  - `[data-mobile-pan-pass-through="true"]` 應允許 `touch-action: pan-x pan-y` 或等效。
...
2026-07-17 canvas CTA pass-through hotfix:
- Root cause: `useMobilePanBroker` 將一般 button 視為 primary action target；欄位內新增任務、看板尾端新增 CTA、TaskWorkbench 未歸位新增 CTA 都屬大型畫布 CTA，卻未標示 pass-through，導致使用者按到這些區域時無法平移畫面。
- Product contract: 大型畫布 CTA 是 tap action，也是 short-pan pan surface；精準控制如 input、select、filter popover、modal、action rail、日期/依賴/負責人控制仍是 pure interactive control。
- Implementation: `BoardView`、`KanbanColumn`、`TaskWorkbenchPanel` 的大型新增 CTA 加上 `data-mobile-pan-pass-through="true"`，CSS 補 `[data-mobile-pan-pass-through="true"] { touch-action: pan-x pan-y; }`。
- Verification: DEV-029 static gate 擴充到 38/38；browser matrix 新增 `QA-029-B10` 欄位新增任務水平短滑、`QA-029-B11` 欄位新增任務垂直短滑、`QA-029-B12` 看板尾端新增 CTA 水平短滑，均需證明可 pan 且不新增任務/欄位、不開 modal、不進 action rail。
...
- Browser evidence that large canvas CTA short pan scrolls board/column and suppresses CTA click-through.
