# SPEC-117：會議紀錄五模式不中斷與 Session 連續性
...
DEV-117 五模式功能與 Targeted QC PASS 是已完成歷史事實；本文件中的 DEV-116 store-owned-policy amendment 是
下一個 DEV-116 candidate 的 target contract，目前尚未修改既有 DEV-117 產品碼，也不回寫其歷史 PASS。
...
成功結果：桌機／筆電使用者可從看板、清單、心智圖、甘特或日曆開始同一場會議，會議中任意切換
這五種 view，右側會議紀錄、草稿、workflow、local recovery 與 live capture segment 均不中斷。

使用思考習慣：#目的、#系統描繪、#限制條件

## 2. Human Decision Brief 與 Spec Impact

- Human confirmed：會議紀錄應可在截圖中的五種模式使用；會議中切換模式不得中斷會議。
- Current phase interpretation：只涵蓋同一 active workspace／board 的五個 task projections。
...
- Rejected：跨 board／workspace meeting、手機 meeting、在五模式各複製一套 meeting store／sidebar／capture。
- 2026-09-10 DEV-116 compatibility amendment：DEV-116 已升級為 RD Implementation Ready／架構已定案，current phase 明確不將
  `goal` 加入五 view allowlist。非 meeting 可使用 goal；從 goal 開始 meeting 沿用 Board fallback；live meeting
  的 selectable options 不含 goal，recovery 遇 persisted goal 先正規化為 Board。DEV-116 Architecture Closure
  Review即為所需amendment：target保留store ownership並只匯出pure predicate；尚未實作，DEV-117已完成
  candidate仍維持store-private baseline與既有QC事實。

## 3. Scope

### 3.1 In Scope

- 五個 continuity views：`board`、`list`、`mindmap`、`gantt`、`calendar`。
- 從任一 continuity view 啟動 live meeting 時保留原 view。
- live meeting 中以既有 ModeSwitcher 切換五個 views，不退出、不保存、不重建、不切段。
...
- DEV-116 `goal` 的 live meeting continuity；current phase 只支援非 meeting goal reading。
...
| meeting UI | `MainLayout` 外層的 `RecordSidebar` | 五 view 切換時保持 mounted |
...
DEV-117已完成baseline：五個continuity views由`src/store/useRecordStore.ts`內的readonly set與private pure
predicate唯一管理，因當時只有meeting start consumer而不新增helper。
...
- DEV-116 `goal` 屬上述其他 view：從 goal 開始 meeting 先切回 Board；若 recovery 讀到 persisted goal，
  先正規化為 Board，再恢復 meeting UI。
...
正常起點：目前 active board 已載入，使用者位於五個 continuity views 任一處；topbar 顯示既有
`新增會議記錄` 或 `紀錄中`，右側使用既有 `RecordSidebar`。

| View | 正常入口 | 會議中 ModeSwitcher | RecordSidebar | Task Details／人工補記 | Live capture | 專屬例外 |
|---|---|---|---|---|---|---|
| Board | 目前 topbar meeting entry | 可切五模式 | 持續 | 沿用 | 沿用 | reservation 可見／可編輯 |
| List | 同一入口；不跳 Board | 可切五模式 | 持續 | 沿用 | 既有 persistence-confirmed mutation | 無 reservation |
| Mindmap | 同一入口；不跳 Board | 可切五模式 | 持續 | 沿用 | 既有 persistence-confirmed mutation | 無 reservation |
| Gantt | 同一入口；不跳 Board | 可切五模式 | 持續 | 沿用 | 既有 persistence-confirmed mutation | 無 reservation |
| Calendar | 同一入口；不跳 Board | 可切五模式 | 持續 | 沿用 | 既有 persistence-confirmed mutation | 無 reservation |

UI rules：

- live meeting 本身不再 disable ModeSwitcher；dependency selection 或 record task-selection 仍可 disable。
- live meeting 的 selectable options 只含五個 continuity views；DEV-116 goal current phase 不顯示，
  不採 disabled／no-op／helper 文案變體。
...
| `scripts/verify-dev-117-cross-mode-meeting-continuity-browser.pw.js` | 真實入口、五模式 sequence、state probe、capture／quick-note／viewport evidence |
...
| `src/components/MainLayout.tsx` | meeting不再是disabled reason；沿用既有五 view options與更新後 copy | 不重做topbar／ModeSwitcher／RecordSidebar layout |
...
- `src/App.tsx`：DEV-117五 view projection與global sidebar mount不改；DEV-116另加goal case與record loaded truth。
...
### 7.4 DEV-116 Target Amendment（NOT IMPLEMENTED）

- `src/store/useRecordStore.ts`保留private readonly set並export `isMeetingContinuityView()`；start non-continuity
  fallback不變，restore遇goal先Board；draft／snapshot schema／capture aggregate不變。
- `src/components/MainLayout.tsx`import store-owned predicate移除live goal option並防禦性正規化非continuity live view；
  ModeSwitcher generic contract不變。
- `scripts/verify-dev-117-cross-mode-meeting-continuity.ts`更新source authority並加入goal negative；歷史artifact不
  重寫，DEV-116 candidate須產生新revision回歸證據。
...
2. **WP-117-B — Store-local policy**：建立唯一private pure predicate；五views原地開始，其他可到達views
...
3. **WP-117-C — Navigation behavior**：`MainLayout`只以dependency／task-selection禁止switch；沿用既有
   五模式 options，五模式 change 直接 `setView`。
...
6. **WP-117-F — Targeted QC**：只對 frozen candidate 執行五模式 UI、state probe、代表性 capture、visible-error 與 viewport evidence。

實作模型可自行決定：test fixture名稱、局部變數名與不改契約的internal type alias；DEV-116不得另行抽出policy module。

實作模型不得決定：增加第六個 continuity view、放行 board/workspace、修改 mobile availability、擴張 reservation、
...
- AC-117-01：從 Board／List／Mindmap／Gantt／Calendar 各自點既有 meeting entry，meeting 在原 view 開始。
- AC-117-02：live meeting 依序 `board → list → mindmap → gantt → calendar → board`，全程可操作且沒有 exit／save dialog。
...
- AC-117-06：五 views 都可由既有入口開 Task Details；active meeting 的 DEV-108 人工補記直接加入同一
...
- Static／pure：DEV-117歷史candidate驗store-local continuity policy、start fallback、disabled predicate與protected
  boundaries；DEV-116 exported predicate／goal negative由QA-DEV-116在新candidate重驗。
...
`Future Phase Captured / Not Requested`：完整五模式專屬操作等價。
...
DEV-116 目標模式 current phase 已完成相容決策：`goal` 不加入 `MEETING_CONTINUITY_VIEWS`，從 goal 開始
meeting 採 Board fallback，live meeting 不顯示 goal option，recovery 遇 persisted goal 先正規化為 Board。
若 future phase 要加入，必須再次 amendment ADR-049／SPEC-117、UI matrix、normal-entry browser sequence 與
capture／quick-note／reservation negative cases，不得只新增 menu option。
...
- DEV-116 target amendment 目前只保留 compatibility decision；產品仍是 DEV-117 store-private baseline。
  DEV-116實作後由store匯出predicate並重跑相容回歸；session、snapshot、capture authority均不變。
- ADR-049 Accepted；P0／P1 readiness gap = 0。
- 結論：DEV-117為`Implemented / Targeted QC PASS / NOT RELEASED`；DEV-116 amendment為
  `RD Implementation Ready / NOT IMPLEMENTED`。文件amendment不等於DEV-116產品已實作或可release。
...
- 2026-09-10：DEV-116 Architecture Closure Review新增goal start／recovery／live-option negative，不改五模式continuity或meeting aggregate。
