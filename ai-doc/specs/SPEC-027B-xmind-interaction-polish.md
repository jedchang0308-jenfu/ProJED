...
日期: 2026-06-19
狀態: Implemented / Browser QC Passed / DEV-028 Detail-Only Title Edit Aligned
父層 DEV: DEV-027
...
本規格以「新增後只選取，不立即進入編輯」作為鍵盤互動核心。舊版「新增後自動開啟 rename input」、「rename input 內 Enter/Tab 連續新增」與「選取後直接打字外層改名」不再是需求，且不得作為驗收條件。
...
- 任務被選取且非編輯中時，直接輸入一般文字鍵不得進入外層 rename mode。
- 心智圖不得建立或顯示 `data-mindmap-title-input`。
- 使用者需要命名或改名時，系統應開啟任務詳情並使用 `TaskDetailsModal` title input。
- `Enter` / `Tab` / 方向鍵在心智圖選取狀態仍維持新增同階、新增子階與移動選取，不得被外層 rename 行為攔截。
