...
- 個人工作紀錄有關聯任務時保留 `關聯任務` 管理區塊；會議模式不呈現關聯任務管理入口，會議空白狀態也不顯示 `0 / 未選取` 摘要或 `選取任務` action。
...
- 新會議標題預設為 `會議紀錄`，不附加日期或時間；`紀錄時間` 以 `YYYY/MM/DD HH:mm` 24 小時制文字格式呈現，不顯示上午／下午。
...
- 會議模式不顯示 `關聯任務` 管理入口、`關聯任務 0 未選取` 摘要或空白狀態 `選取任務` action；個人工作紀錄的任務選取入口與已有關聯任務的管理、角色調整維持。
...
4. `存草稿` 移到會議內容編輯器下方的獨立次要文字按鈕；流程列保留唯一主要 `發布` 入口，不把存草稿藏在 `速記` step 或狀態／分享範圍控制列；空白會議草稿仍可保存，既有 baseline、checkpoint 與離開防呆維持。
...
## 2026-09-01 會議流程再精簡 addendum

- 依使用者最新 UI 留言，會議 workflow 不再呈現 `匯入` 與 `校稿` step；畫面只保留 `速記 → AI整理 → 發布`。
- `匯入` 改由流程列下方唯一的 `帶入上次會議後變更` 控制項提供；`校稿` 不再以文字、icon、按鈕或可存取 step 名稱出現在會議流程列，AI整理後仍可直接編輯內容，並使用內容區下方的 `存草稿` 保存草稿。
- 既有 review 狀態與資料模型可保留供保存／發布邏輯使用，但不得回到會議 workflow 的可見 UI；發布仍是流程列唯一的正式發布入口。

## 2026-09-01 會議匯入雙層揭露 addendum

- 會議內容區第一層改為單一 `匯入專案變化` 入口，維持 `內容` 標題右側的既有第一層排版。
- 點擊後以覆蓋式第二層提供 `帶入上次會議後變更` 與 `自訂日期`；預設與自訂不再於第一層並列常駐。
- `自訂日期` 的日期欄位與 `帶入` 動作在第二層內切換顯示，可返回選項清單或關閉；本次只改可見互動揭露，不改匯入 query、metadata、去重、錯誤或保存契約。

## 2026-09-01 會議底部控制列精簡 addendum

- 移除 `目前狀態／草稿` 摘要；會議流程列、內容區保存回饋與 `存草稿／發布` 動作已提供必要的流程與結果訊號，底部控制列將 `存草稿` 與可操作的分享範圍合併為同一列。
...
- 保留舉證：獨立 `存草稿` 必須保留，否則使用者無法明確建立可在紀錄庫續寫的 durable draft；最小形式是內容編輯器下方的文字按鈕，不新增說明卡。
...
- 會議模式內容編輯器下方獨立 `存草稿` 操作及其保存中、成功、失敗回饋；發布由 workflow 唯一入口負責。
...
- meeting mode 在內容編輯器下方保留 `data-record-meeting-actions` 的次要 `存草稿`；它共用既有 `saveDraft({nodes,status})` 與 single-flight／disabled reason，不建立第二套 persistence。發布只保留 workflow 入口並呼叫同一 handler；`速記` 不再承擔儲存。
- 1440×900 與 390×844 都必須讓 editor、custom dates 與內容下方的存草稿 action 可見／可捲到，不得水平 overflow、重疊或裁切；390 仍遵守 SPEC-069，若產品邊界不 render meeting UI，驗證應是負向不存在，不得為本 DEV 解禁手機會議功能。
