...
| 2 | Phase 2 Local Source Implemented / Remote DB-Edge-Production Not Executed | Supabase preview + feed v2 | filters_json v2 validation migration source、Edge Function v2 query source、ICS feed parity static contract |
| 3 | Authorized / Release-Gate Blocked | Production release and live smoke | migration apply、Edge deploy、Firebase deploy、live `.ics` smoke |
...
| Production release / live smoke | Same Spec Phase | Phase 3 | Phase 2 local gates and local DB smoke pass; remote apply/deploy/live smoke still require deployment-release-gate / Supabase gate |
...
| Phase 3 | Authorized / Remote Gate Pending | RD Contract Ready / Remote Readiness Static + Local DB Smoke Passed | Production migration/function/frontend deploy and live smoke | Unscoped feature additions | Phase 2 passed + deployment authorization + local DB smoke passed + deployment-release-gate / Supabase gate | Production link outputs only allowed previewed tasks | remote readiness preflight, local DB smoke, deployment-release-gate, live feed smoke |
