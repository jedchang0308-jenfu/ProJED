...
## 14. 統一過濾器選單與狀態提升 (Unified Filter Menu & State Hoisting)

### 14.1 設計意圖
- **視覺降噪 (Visual Noise Reduction)**：解決隨功能增加導致工具列按鈕（如狀態過濾、依賴線開關、層級切換）過度擁擠的問題。
- **單一真相來源 (Single Source of Truth)**：消除各模式（清單、看板、甘特、月曆）中重複定義的過濾邏輯與 UI 模板。
...
- **介面優化**：
    ...
    - **互動防護**：實作 `handleClickOutside` 自動關閉邏輯，並確保 Z-Index 足以覆蓋所有視圖內容（甘特圖網格、月曆格線）。
...
## 16. 跨模式依賴關係選取優化 (Cross-View Dependency Selection Optimization)

### 16.1 設計意圖
- **操作一致性**：消除看板模式與清單模式之間設定依賴關係的介面差異，提供統一的操作直覺。
- **視覺引導強化**：透過顯眼的選取狀態提示（Banner、準星游標、來源高亮），確保使用者在建立複雜邏輯連線時不會迷失。

### 16.2 架構決策
- **入口集中化 (Unified Context Menu)**：
    - 將依賴關係的觸發點由原先的分散按鈕（清單行內按鈕）統一整合至全域右鍵選單 (`GlobalContextMenu`)。
    - **效益**：移除清單模式中干擾視覺的行內 UI，並使「看板」與「清單」的操作流程 100% 同步。
- **選取模式交互規範 (Selection Workflow)**：
    - **日期占位符 (`...`)**：在選取模式下，即使任務尚未設定日期，也會強制顯示帶有 `...` 的互動按鈕。這允許使用者在規劃初期（尚未排定具體日期時）就能先建立任務間的邏輯結構。
    - **來源節點保護**：選取模式啟動後，來源節點（Source）會以琥珀色環 (Ring) 標示，並對其點擊行為進行防護，防止重複選取。
- **流程中斷保護 (View Switching Guard)**：
    - **狀態聯動**：在 `MainLayout` 中監聽 `dependencySelection` 狀態。當選取模式啟動時，自動將頂部視圖切換按鈕設為 `disabled` 並顯示提示資訊。
    - **設計意圖**：防止使用者在選取過程中因切換視圖（如從看板切換到月曆）而導致選取橫幅遺失、操作狀態斷裂或資料不一致。
...
## 18. 手機端體驗優化與輸入防護 (Mobile UX & Input Protection)

### 18.1 手機端長按識別機制 (`useLongPress.ts`)
- **設計意圖**：解決手機瀏覽器原生長按選單（Callout）與拖曳手勢衝突的問題。
- **實現方式**：
    - 建立 `useLongPress` Hook，利用獨立計時器（500ms）識別長按行為。
    - **防衝突邏輯**：長按判定時間（500ms）設定為長於拖曳延遲。若手指在計時期間位移超過 8px，則立即取消長按計時，確保不會誤發右鍵選單。
    - **原生攔截**：在 `index.css` 針對 `.touch-none` 元素強制套用 `-webkit-touch-callout: none`，永久阻止系統原生氣泡選單干擾 Web App 互動。

### 18.2 手機端拖曳政策 (Mobile Drag-and-Drop Policy)
- **架構決策**：考量到觸控端精確排序與階層管理的複雜度，以及防止頁面捲動時的誤觸，決定**完全停用手機端的觸控拖曳 (TouchSensor)**。
- **效益**：顯著提升手機端瀏覽與長按選單的穩定性。使用者若需進行複雜的 WBS 結構調整，建議切換至電腦版操作。

### 18.3 集中式輸入防護 (`SmartKeyboardSensor`)
- **設計意圖**：解決 `dnd-kit` 的鍵盤感應器會攔截 `Space` 與 `Enter` 鍵，導致在行內編輯時打字會意外觸發拖曳或跳號的問題。
- **實現方式**：
    - **非侵入式攔截**：不使用分散在各元件的 `e.stopPropagation()`，而是擴充 `KeyboardSensor` 建立 `SmartKeyboardSensor`。
    - **自動過濾**：在 Sensor 層級自動偵測事件來源。若 `target` 為 `INPUT`, `TEXTAREA` 等互動元素，則直接返回 `false` 阻止感應器啟動。
    - **零技術債**：這是一次性的全域保護。未來新增任何編輯欄位，只要它是標準表單標籤，都會自動受到保護，無需額外撰寫防禦程式碼。

---
*更新日期：2026-05-06 (手機長按選單、停用手機拖曳、集中式鍵盤防護實作)*
